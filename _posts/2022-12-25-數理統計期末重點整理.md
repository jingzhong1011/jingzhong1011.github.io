...
$T(\underline{X})$：樣本的函數  
If $X_1\dots X_n \overset{\text{iid}}{\sim} f(x)$ then,  
1. $E(\bar{X})=\mu$
...
The distribution of $T=(T_1,\dots ,T_k)$,
 $f_T(u_1,\dots,u_k)=H(u_1,\dots,u_k)[c(\theta)]^n\exp\{\sum^k_{j=1}w_j(\theta)u_j(x)\}$
...
A sequence of r.v. $Y_1 \dots Y_n$ converges in probability to a r.v. $Y$ if $\forall \epsilon>0$, 
$\lim_{n\rightarrow\infty}P(|Y_n-Y|<\epsilon)=1$ denoted by $Y_n\overset{p}{\rightarrow}Y$
...
Then, $\bar{X}_n\overset{p}{\rightarrow}\mu$
...
- $X_n\overset{d}{\rightarrow}c\Rightarrow X_n\overset{p}{\rightarrow}c$
- if $X_n\overset{d}{\rightarrow}X, Y_n\overset{p}{\rightarrow}a$,  
